...
class PartialTopSurfaceMirror(pv.FresnelSurfaceDelegate):
    ...
    def reflectivity(self, surface, ray, geometry, container, adjacent):
        ...
        if geometry.radius == d_holder / 2 and geometry.length == h_holder:  # real holder
            # print(z, h_holder)
            if np.isclose(z, h_holder / 2) and np.abs(x) < d_holder / 2 and np.abs(y) < d_holder / 2:
                return super(PartialTopSurfaceMirror, self).reflectivity(surface, ray, geometry, container,
                                                                         adjacent)
            elif np.isclose(z, -h_holder / 2) and np.abs(x) < d_bottom_hole / 2 and np.abs(y) < d_bottom_hole / 2:
                return super(PartialTopSurfaceMirror, self).reflectivity(surface, ray, geometry, container,
                                                                         adjacent)
            return 1
...
if __name__ == '__main__':
    print('hi')
    scene = pv_scene_real(absor=0.05, scat=0.1)
    positions = cs.scene_render_and_positions(scene, rays_number=500, show_3d=1)
    exit()
    x_res, y_res, z_res = 191, 191, 191
    xM = -1, 1
    yM = -1, 1
    zM = -1, 1
