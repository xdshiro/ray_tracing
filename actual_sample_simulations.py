...
class PartialTopSurfaceMirror(pv.FresnelSurfaceDelegate):
    ...
    def reflectivity(self, surface, ray, geometry, container, adjacent):
        ...
        print(z)
        if np.isclose(z, 2) and np.abs(x) < 1 and np.abs(y) < 1:
            return super(PartialTopSurfaceMirror, self).reflectivity(surface, ray, geometry, container,
                                                                     adjacent)
        elif np.isclose(z, -2) and np.abs(x) < 1 and np.abs(y) < 1 :
            return super(PartialTopSurfaceMirror, self).reflectivity(surface, ray, geometry, container,
                                                                     adjacent)
        else:
            # ray.alive = False
...
if __name__ == '__main__':
    print('hi')
    scene = pv_scene_real(absor=0.15, scat=1.5)
    positions = cs.scene_render_and_positions(scene, rays_number=5000, show_3d=1)
    # exit()
